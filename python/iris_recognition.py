...
input_img_path = "/Users/leejonguk/Desktop/hanyang/졸프/iris/CASIA-IrisV4(JPG)/CASIA-Iris-Interval"
output_path = "/Users/leejonguk/Desktop/temp"
iris_circle = [0, 0, 0]
...
def getPolar2CartImg(image, rad):
	...
	print(np.size(image,0), np.size(image, 1))
	...
	for valid_width in reversed(range(rad*3)):
		...
		for h in range(360):
			...
			if (imgRes[h][valid_width] != 0):
				blank_cnt+=1
		if(blank_cnt == 0):
			imgRet = imgRes[0:360, valid_width:rad*3]
			break

	imgRet = cv.resize(imgRet, (80, 360), interpolation=cv.INTER_LINEAR)
